...
EXPORT_DIR=${SCRIPT_DIR}/../libtensorflow-cc
...
echo "Building ${IMAGE} ... "
docker build --build-arg TF_VERSION=${TF_VERSION} --build-arg JOBS=${JOBS} --build-arg GPU_POSTFIX=${GPU_POSTFIX} --target ${STAGE} -t ${IMAGE} ${BUILD_DIR} | tee ${LOG_FILE}
